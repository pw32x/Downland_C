...
const dl_u8 roomToBankIndex[] = 
{
    ...
    5,
    6,  // title screen
	1,  // transition screen. it doesn't matter what the bank is.
	1,  // wipe transition screen. it doesn't matter what the bank is.
	6   // get ready screen
};

#define SPRITE_BANK_NUMBER 7
...
dl_u8 currentbank = 1;
#define MYSG_mapROMBank(n) currentbank = n; SG_mapROMBank(n)


#define SG_saveROMBank()       unsigned char _saved_slot2_ROM_bank = currentbank
#define SG_restoreROMBank()    MYSG_mapROMBank(_saved_slot2_ROM_bank)
...
void chamber_draw(dl_u8 roomNumber)
{
	...
	SG_saveROMBank();
	MYSG_mapROMBank(SPRITE_BANK_NUMBER);
...
void transition_init(const Room* targetRoom)
{
	...
	MYSG_mapROMBank(roomToBankIndex[gameData_transitionRoomNumber]);
