...
This is the Sega 32X port of Downland_C.
...
- This repository
- A copy of the Chilly Willy 32X sdk (https://github.com/viciious/32XDK/releases)
- A copy of the Downland V1.1 rom placed in the gamedata folder
...
- download Chilly Willy 32X Devdit
- run build.bat in the build folder or use the provided Visual Studio solution.
- the Downland.32x file will be in the out folder 
   

## RUNNING THE GAME

On hardware you'll need: 
- an NTSC Sega Genesis with 32X (PAL will work but will run slower, naturally)
- a flashcard with the Downland.32x file

Otherwise
- an emulator (there are very many)
- the built downland.32x as a file 
