                                          ...
                                          enum CrtColor crtColor) 
{
    ...
    for (int y = 0; y < height; ++y) 
    {
        ...
        for (int x = 0; x < width; x += 2) 
        {
            dl_u8 leftPixel = vram[yOffset + x];
            dl_u8 rightPixel = vram[yOffset + x + 1];

            if (rightPixel == BLUE && x < width - 2)
            {
                dl_u8 pixel3 = vram[yOffset + x + 2];
                if (pixel3 == ORANGE || pixel3 == WHITE)
                {
                    rightPixel = WHITE;
                    leftPixel = BLACK;
            ...
            vram[yOffset + x] = leftPixel;
            vram[yOffset + x + 1] = rightPixel;
        }
        */
    }
}
